plot.Node <- function(x, pch=20, data="no",...){
  ...
  if(data=="yes"){
    ...
    return(plot(nodes, pch=8, col=2, add=TRUE))
  }
  if(data=="only"){
    nodesWithData<-unique(x[[1]]$id)
    nodes<-findID(x, nodesWithData)
    return(plot(nodes, pch=pch, ...))
  }
}
...
plot.OSM<-function(x, nodes=FALSE, ...){
  if(class(x$Way[[2]])=="SpatialLinesDataFrame"){
    plot(x$Way,...)
    if(nodes==TRUE)
      plot(x$Node, data="only", col=2, add=TRUE)
  } else {
